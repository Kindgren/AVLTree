...
public class AVLTreeNode<T extends Comparable<T>> {
	...
	if (data.compareTo(element) < 0) {
		...
		//if (left.getElement().compareTo(right.getElement())>1) {
		
	  if (nodeHeight(left) - nodeHeight(right) >1 || nodeHeight(left) - nodeHeight(right) < -1) {
    ...
    private AVLTreeNode<T> balanceTree(AVLTreeNode<T> node) {
        ...
        if (nodeHeight(left) - nodeHeight(right) == 2) {
                ...
                if (nodeHeight(left.right) < 0) {
                    return singleRotationWithLeftChild(node); // singelwithleftchild
                } else {
                    return doubleRotationWithLeftChild(node); // doublewithleftChild
                }
     
            } else if (nodeHeight(left) - nodeHeight(right) == -2) {
     
                if (nodeHeight(right.left) < 0) {
     
                    return singleRotationWithRightChild(node);
     
                } else {
                    return doubleRotationWithRightChild(node); // doublewithrightChild
                }
